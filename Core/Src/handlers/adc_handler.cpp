...
bool adc_start_system_monitor(uint16_t rev_mv)
{
    ...
    if (adc::enable(ADC2))
    {
        ...
        if (adc::set_sampling(ADC2, COIL_CURRENT_ADC_CHANNEL, SMP_7_5_cycles) &&        // Ток катушки      - 2й канал, по watchdog`у без инжектируемого преобразования
